...
* **Software**: the dashboard is produced using `Rmarkdown` but the inputs are just datas. If you work in stata or anything else and can produce `.dta` files (best to save as version 13 or below), `.csv` or `xls` it should be easy to input those to this set up. 

* **Hosting**: If you can get the input files to us and can regularly upload your data to github we think we can produce and host the dashboard on this site and catalogue it with other ones. Alternatively the package produces a htm file that you can host anywhere. Contact us at:...
