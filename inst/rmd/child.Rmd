...
p_sympt
```
```{r, eval=(my_subset$family[[1]]=="Knowledge about cure"), echo = FALSE, message = FALSE, warning = FALSE, results = 'hide', fig.height = 3, fig.width = 6}
...
cure <- df[,c(which(colnames(df)=='Lemon_water'):which(colnames(df)=='cure_777'))]  %>% names  #exclude others
df_cure <-  data.frame(
...
p_fol_gov <- ggplot(fol_gov, aes(x=`Follow Guide`,y=perc)) +
  ...
  theme(axis.text.x = element_text(colour="black"), 
        axis.text.y = element_text(colour="black")) +
  ggtitle("Out of 10 people in your village, \n how many do you think follow the gov’t guidelines?")
