...
plot_aggregates(my_data, 
                filter(my_vars, family == fam & !is.na(variable)))
...
vars <- filter(my_vars, family == fam & !is.na(variable)) %>% pull(variable)
...
if (!is.null(my_vars$min) & !is.null(my_vars$max)){
plot <- plot + geom_blank(aes(x=date, y=ceiling)) + 
