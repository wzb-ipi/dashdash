...
```{r, eval=(aggregate) && nrow(my_subset)==2, echo = FALSE, fig.height = nrow(my_subset)*1.42 + 1, message = FALSE, warning = FALSE, results = 'hide'}
all_bar_plot(my_data, my_subset, nrow=1)
```

```{r, eval=(aggregate) && nrow(my_subset)>=10, echo = FALSE, fig.height = nrow(my_subset)*1.42 + 1, message = FALSE, warning = FALSE, results = 'hide'}
all_bar_plot(my_data, my_subset, nrow=10)
```

```{r, eval=(aggregate) && nrow(my_subset)<10 && nrow(my_subset)>4, echo = FALSE, fig.height = nrow(my_subset)*1.42 + 1, message = FALSE, warning = FALSE, results = 'hide'}
all_bar_plot(my_data, my_subset, nrow=4)
```

```{r, eval=(aggregate) && nrow(my_subset)==4, echo = FALSE, fig.height = nrow(my_subset)*1.42 + 1, message = FALSE, warning = FALSE, results = 'hide'}
